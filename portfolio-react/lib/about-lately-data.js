...
async function getGitHub(env) {
  const username = env.GITHUB_USERNAME || "hushbrush";
  const headers = {
    Accept: "application/vnd.github+json",
    "User-Agent": "portfolio-about-lately",
  };
  if (isUsableSecret(env.GITHUB_TOKEN)) headers.Authorization = `Bearer ${env.GITHUB_TOKEN}`;
...
export async function createAboutLatelyData(env = process.env) {
  const [weather, listening, building, moving] = await Promise.all([
    getWeather().catch(() => fallback.weather),
    getSpotify(env).catch(() => fallback.listening),
    getGitHub(env).catch(() => fallback.building),
    getStrava(env).catch(() => fallback.moving),
  ]);

  return { weather, listening, building, moving };
}
